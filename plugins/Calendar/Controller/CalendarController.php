...
class CalendarController extends BaseController
{
    ...
    public function userEvents()
    {
        ...
        //GET HOLIDAY
        //QUERY Service Provide app\serviceprovider\FilterProvider.php
        //create function, register in construct and create Model
        $startTime = strtotime($startRange);
        $endTime = strtotime($endRange);
        $holidays = $this->pluginLoader->holiday->getAll($startTime, $endTime);
        if(!empty($holidays)){
            foreach ($holidays as $key => $value) {
                $holidays[$key]["start"] = date("Y-m-d h:i:s",$value["start"]);
                $holidays[$key]["end"] = date("Y-m-d h:i:s",$value["end"]);
            }
        }
